...
from utils.utils import Config, Logger, format_time, print_model_spec, get_git_hash
...
class Trainer:
    ...
    def test(self):
        ...
        with torch.no_grad():
            for iteration, batch in tqdm(
                enumerate(self.val_dataloader), total=len(self.val_dataloader)
            ):
    ...
    def print_log(self, loss, step_duration, data_fetch_time, model_update_time):
        ...
        training_time_left = (
            self.config.n_iterations / self.global_step - 1.0
        ) * time_so_far
        print_string = (
            "Iteration {:>6}/{} | examples/s: {:5.1f}"
